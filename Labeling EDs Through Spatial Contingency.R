...
#Create List of Neighbors
 ...
 while (Start!=End){
  ...
  #Attach Results to Original Shapefile and Output
  setwd("C:/Users/mmarti24/Google Drive/Maps/Shape Files/San Antonio 1930-40") #Work
  sa_choice<-readOGR(dsn=getwd(), layer="SanAntonio_1930_ED_Choice_Map_PostEdit", stringsAsFactors = F) #Shapefile
  
  neighs<-plyr::rename(neighs, c(FirstE="Rook_C"))
  myvars<-c("pblk_id", "Rook_C")
  neighs2<-neighs[myvars]
  
  sa_choice<-merge(x=sa_choice, y=neighs2, by="pblk_id", all.x=T)
  writeOGR(sa_choice, dsn=getwd(), "SanAntonio_1930_ED_Choice_Map_RookChoices",check_exists = TRUE,
