...
for (i in 1:length(stratum.counts.to.merge$enumdist_pop)) { ######work on this!!!
  if (stratum.counts.to.merge[i,3] >= 1727) {   ##see the acs script for where this # comes from, based on ACS empirical distribution
    sample.sizes[i] = ceiling((1/8)*sample.sizes[i])  ##Note that these are (probably) not the right fractions
  } else {
    sample.sizes[i] = ceiling((1/6)*sample.sizes[i])
  } 
}
...
save(edcor,file="corr_ed_cxstr2_10k.Rdata")
save(coefs.ed,file="coefs_ed_cxstr2_10k.Rdata")
save(res.ed,file="res_ed_cxstr2_10k.Rdata")
save(fits.ed,file="fits_ed_cxstr2_10k.Rdata")
